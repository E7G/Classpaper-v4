...
以下是前端配置文件 `res/config/config.js` 中所有可配置项的详细说明：

##### 📚 课程表配置 (`CONFIG.lessons`)

| 🔧 配置项 | 📝 说明 | 💡 示例 |
| --------- | ------- | ------- |
| `headers` | 表头显示 | `["星期", "1", "2", ..., "11"]` |
| `displayMode` | 显示模式 | `"scroll"` 或 `"day"` |
| `schedule` | 课程安排 | 按星期划分的课程数组 |
| `times` | 时间安排 | 每节课的起止时间 |

##### 🕐 时间安排配置 (`CONFIG.lessons.times`)

| 🔧 配置项 | 📝 说明 | 💡 示例 |
| --------- | ------- | ------- |
| `semester.begin` | 学期开始日期 | `"2023-07-31"` |
| `semester.end` | 学期结束日期 | `"2026-06-07"` |
| `schedule[].period` | 课节编号 | `1, 2, 3, ..., 11` |
| `schedule[].begin` | 开始时间 | `"07:20"` |
| `schedule[].end` | 结束时间 | `"07:55"` |
| `schedule[].rest` | 休息时间 | `"07:55-08:00"` |

##### 📅 周偏移配置 (`CONFIG.weekOffset`)

| 🔧 配置项 | 📝 说明 | 💡 示例 |
| --------- | ------- | ------- |
| `enabled` | 是否启用周偏移 | `true` 或 `false` |
| `offset` | 偏移周数 | `7`（表示第7周） |
...
| 🔧 配置项 | 📝 说明 | 💡 示例 |
| --------- | ------- | ------- |
| `enabled` | 是否启用通知 | `true` 或 `false` |
| `regularInterval` | 常规提醒间隔（分钟） | `5` |
| `endingTime` | 结束前提醒时间（分钟） | `5` |
...
const CONFIG = {
  "lessons": {
    ...
    "schedule": [
      {
        "day": "周一",
        "classes": ["升旗", "班会", "物理", "英语", "数学", "数学", "生物", "化学", "语文", "晚修", "晚修"]
      }
      // ... 其他星期配置
    ],
    "times": {
...
我们正在积极开发全新的UI界面，提供更现代、更流畅的用户体验。新UI目前位于 `dev` 分支，包含以下改进：

#### 🎨 界面优化

- **响应式设计**：适配各种屏幕尺寸，从笔记本到大屏显示器
- **现代化布局**：采用最新的设计语言，提供更直观的操作体验
- **主题系统**：支持浅色/深色主题，自动适应系统设置（莫奈取色，已简单实现）
- **动画效果**：流畅的过渡动画，提升交互体验

#### 🛠️ 功能增强

- **实时预览**：设置修改后即时预览效果
- **拖拽操作**：支持课程表、壁纸等内容的拖拽排序（暂无）
- **数据导入导出**：支持课程表、配置的导入导出功能
...
如果您想提前体验新UI功能，可直接前往release页面下载最新的beta版本。

### ⚠️ 注意事项

- **dev分支**：包含开发中的功能，可能存在不稳定因素
- **数据兼容性**：新UI可能与旧版本配置文件不完全兼容
- **反馈渠道**：遇到问题请通过 [GitHub Issues](https://github.com/E7G/Classpaper-v4/issues) 反馈
- **回滚方法**：如需回到稳定版本，请切换回 `main` 分支
