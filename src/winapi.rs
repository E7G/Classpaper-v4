...
use winapi::um::{
    dwmapi::{DwmSetWindowAttribute, DwmExtendFrameIntoClientArea, DwmEnableBlurBehindWindow, DWM_BLURBEHIND, DWM_BB_ENABLE, DWM_BB_BLURREGION},
    uxtheme::MARGINS,
    winuser::{
        EnumWindows, FindWindowA, FindWindowExA, FindWindowExW, FindWindowW, GetSystemMetrics, GetWindowTextW, SetParent, ShowWindow, SetWindowPos,
        SendMessageTimeoutA, SendMessageTimeoutW, HWND_BOTTOM, HWND_TOP, SW_HIDE, SW_SHOW, SW_SHOWNORMAL, SWP_NOMOVE, SWP_NOSIZE, SWP_NOACTIVATE, SWP_DRAWFRAME,
        SM_CXSCREEN, SM_CYSCREEN, SMTO_NORMAL, SetProcessDPIAware, GetWindow, GW_HWNDPREV, GetWindowLongPtrW, SetWindowLongPtrW,
        GWL_STYLE, GWL_EXSTYLE, WS_EX_LAYERED, WS_EX_TOOLWINDOW, WS_CHILDWINDOW, WS_POPUP, WS_OVERLAPPED, WS_CAPTION, WS_BORDER,
        WS_SYSMENU, WS_THICKFRAME, SetLayeredWindowAttributes, LWA_ALPHA, AdjustWindowRect, MoveWindow,
        SetWindowPlacement, WINDOWPLACEMENT, WPF_SETMINPOSITION, GetClassNameW, GetWindowThreadProcessId,
    },
...
pub fn raise_desktop(h_progman_wnd: HWND) -> bool {
    ...
    unsafe {
        ...
        let result0 = SendMessageTimeoutW(
            h_progman_wnd, 0x052C, 0xA, 0, SMTO_NORMAL, 1000, &mut res0 as *mut usize
        );
        if result0 == 0 || res0 != 0 {
            println!("[winapi debug] 桌面初始化检查失败: result={}, res={}", result0, res0);
            return false;
        }
        
        // Prepare to generate wallpaper window
        SendMessageTimeoutW(h_progman_wnd, 0x052C, 0xD, 0, SMTO_NORMAL, 1000, &mut res1 as *mut usize);
        SendMessageTimeoutW(h_progman_wnd, 0x052C, 0xD, 1, SMTO_NORMAL, 1000, &mut res2 as *mut usize);
        // "Animate desktop", which will make sure the wallpaper window is there
        SendMessageTimeoutW(h_progman_wnd, 0x052C, 0, 0, SMTO_NORMAL, 1000, &mut res3 as *mut usize);
        
        println!("[winapi debug] RaiseDesktop 结果: res1={}, res2={}, res3={}", res1, res2, res3);
...
pub fn is_explorer_worker(hwnd: HWND) -> bool {
    ...
    unsafe {
        ...
        let class_str = String::from_utf16_lossy(&class_name).trim_matches('\0').to_string();
        ...
        if result > 0 {
            let exe_str = String::from_utf16_lossy(&exe_name).trim_matches('\0').to_lowercase();
...
pub fn setup_window_style(hwnd: HWND) -> bool {
    ...
    unsafe {
        ...
        println!("[winapi debug] 窗口样式设置完成: style={:x}, exstyle={:x}", style_tw, exstyle_tw);
        true
    }
}
...
pub fn setup_fullscreen_position(hwnd: HWND) -> bool {
    ...
    unsafe {
        ...
        MoveWindow(hwnd, rc_full_screen.left, rc_full_screen.top, rcfx, rcfy, TRUE);
        
        let wp = WINDOWPLACEMENT {
            length: mem::size_of::<WINDOWPLACEMENT>() as u32,
            flags: WPF_SETMINPOSITION,
            showCmd: SW_SHOWNORMAL as u32,
            ptMinPosition: winapi::shared::windef::POINT { x: rc_full_screen.left, y: rc_full_screen.top },
            ptMaxPosition: winapi::shared::windef::POINT { x: rc_full_screen.left, y: rc_full_screen.top },
...
pub fn setup_wallpaper_new(window_title: &str) -> bool {
    ...
    println!("[winapi debug] 找到窗口 HWND: {:?}, 开始新版本桌面穿透设置", hwnd);
    ...
    let h_top_desk_wnd = unsafe {
        FindWindowW(
            OsStr::new("Progman").encode_wide().chain(Some(0)).collect::<Vec<u16>>().as_ptr(),
            OsStr::new("Program Manager").encode_wide().chain(Some(0)).collect::<Vec<u16>>().as_ptr(),
        )
    };
    ...
    let mut h_shell_def_view = unsafe {
        FindWindowExW(
            h_top_desk_wnd,
            ptr::null_mut(),
            OsStr::new("SHELLDLL_DefView").encode_wide().chain(Some(0)).collect::<Vec<u16>>().as_ptr(),
            OsStr::new("").encode_wide().chain(Some(0)).collect::<Vec<u16>>().as_ptr(),
        )
    };
    ...
    let mut h_worker = unsafe {
        FindWindowExW(
            h_top_desk_wnd,
            ptr::null_mut(),
            OsStr::new("WorkerW").encode_wide().chain(Some(0)).collect::<Vec<u16>>().as_ptr(),
            OsStr::new("").encode_wide().chain(Some(0)).collect::<Vec<u16>>().as_ptr(),
        )
    };
    ...
    if h_worker.is_null() {
        h_worker = unsafe {
            FindWindowExW(
                h_top_desk_wnd,
                ptr::null_mut(),
                OsStr::new("WorkerA").encode_wide().chain(Some(0)).collect::<Vec<u16>>().as_ptr(),
                OsStr::new("").encode_wide().chain(Some(0)).collect::<Vec<u16>>().as_ptr(),
            )
        };
    }
    ...
    if h_worker.is_null() {
        h_worker = if !h_worker2.is_null() { h_worker2 } else { h_top_desk_wnd };
        is_version_1_2 = true;
    ...
    // 7. 设置父子关系和窗口位置
    unsafe {
        SetParent(hwnd, if is_version_1_2 { h_worker } else { h_top_desk_wnd });
        
        SetWindowPos(hwnd, HWND_TOP, 0, 0, 0, 0, 
            SWP_NOMOVE | SWP_NOSIZE | SWP_NOACTIVATE | SWP_DRAWFRAME);
        SetWindowPos(h_shell_def_view, HWND_TOP, 0, 0, 0, 0,
            SWP_NOMOVE | SWP_NOSIZE | SWP_NOACTIVATE);
        SetWindowPos(h_worker, HWND_BOTTOM, 0, 0, 0, 0,
            SWP_NOMOVE | SWP_NOSIZE | SWP_NOACTIVATE | SWP_DRAWFRAME);
    }
    ...
    thread::spawn(move || {
        ...
        loop {
            ...
            if !ok {
                ...
                if !conflict_hwnd.is_null() {
                    println!("[winapi debug] 检测到冲突窗口: {:?}, 连续修复次数: {}", 
