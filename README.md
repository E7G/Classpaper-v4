...
| 📋 告示设置                                                      | 📖 关于页面                                                        |
| ---------------------------------------------------------------- | ------------------------------------------------------------------ |
| ![setting-sth](res/screenshots/setting-sth.jpeg "设置-告示设置") | ![about](res/screenshots/about.png "关于页面") |
...
| 🔧 配置项 | 📝 说明 | 💡 示例 |
| --------- | ------- | ------- |
| `lessons.headers` | 表头显示 | `["星期", "1", "2", ..., "11"]` |
| `lessons.displayMode` | 显示模式 | `"scroll"` 或 `"day"` |
| `lessons.schedule` | 课程安排 | 按星期划分的课程数组 |
| `lessons.times.semester` | 学期日期 | `{"begin": "2023-07-31", "end": "2026-06-07"}` |
| `lessons.times.schedule` | 时间安排 | `[{"period": 1, "begin": "07:20", "end": "07:55", "rest": "07:55-08:00"}]` |
| `weekOffset` | 周偏移 | `{"enabled": true, "offset": 7}` |
| `notifications` | 通知设置 | `{"enabled": true, "regularInterval": 5, "endingTime": 5}` |
| `events` | 事件列表 | `[{"name": "高考", "date": "2026-06-07T00:00:00"}]` |
| `wallpapers` | 壁纸列表 | `["wallpaper/bg1.jpg", "wallpaper/bg2.jpg"]` |
| `wallpaperInterval` | 切换间隔（分钟） | `30` |
| `progressDescription` | 进度描述 | `"高三剩余"` |
| `progressPercentMode` | 百分比模式 | `"left"` 或 `"passed"` |
| `sth` | 告示内容 | `"一鸣从此始，相望青云端"` |

##### 💬 弹幕留言配置

弹幕留言数据存储在 `res/config/messages.js` 文件中，支持全屏弹幕展示、留言管理、弹幕开关控制。
...
const CONFIG = {
  "lessons": {
    ...
    "displayMode": "scroll",
    "schedule": [
      {"day": "周一", "classes": ["升旗", "班会", "物理", "英语", "数学", "数学", "生物", "化学", "语文", "晚修", "晚修"]}
    ],
    "times": {
      "semester": {"begin": "2023-07-31", "end": "2026-06-07"},
      "schedule": [
        {"period": 1, "begin": "07:20", "end": "07:55", "rest": "07:55-08:00"},
        {"period": 2, "begin": "08:00", "end": "08:40", "rest": "08:40-08:50"}
      ]
    }
  },
  "weekOffset": {"enabled": true, "offset": 7},
  "notifications": {"enabled": true, "regularInterval": 5, "endingTime": 5},
  "events": [
    {"name": "高考", "date": "2026-06-07T00:00:00"},
    {"name": "期末考试", "date": "2024-01-15T00:00:00"}
  ],
  "wallpapers": ["wallpaper/bg1.jpg", "wallpaper/bg2.jpg"],
...
- 本地/远程网页一键展示，托盘菜单支持重载、穿透、重启、设置、退出
- 智能Windows版本适配，自动选择最佳桌面穿透方案，100%兼容性保证
- 增强桌面穿透：窗口样式优化、DWM透明效果、Z顺序监控、线程安全
- 配置文件自动兼容 Go/Rust 格式，支持热重载
- 本地文件禁用缓存确保实时刷新，远程网页默认启用缓存
- 日志系统美观、支持多级别、自动降级输出
- 多窗口管理，主窗口/设置窗口均可严格关闭
- 支持壁纸轮换、进度条、事件提醒、弹幕留言
- 智能错误通知：Windows原生弹窗，提供详细错误原因和解决建议
- 弹幕留言：全屏弹幕展示、留言板管理、弹幕开关控制、自动循环播放
...
ClassPaper v4 新增了**智能错误通知系统**，通过Windows原生弹窗提供详细的错误信息和解决建议。

#### 常见错误场景

**1. 桌面穿透设置失败**

- **解决方案**：以管理员身份运行程序，检查是否有其他壁纸软件占用桌面，查看 `app.log` 获取详细错误信息

**2. 配置文件读写错误**

- **解决方案**：检查 `config.toml` 文件权限，确保磁盘空间充足，程序会自动使用默认配置继续运行

**3. 系统托盘创建失败**

- **解决方案**：程序会自动重试一次，无托盘功能时仍可通过任务管理器操作

**4. 文件操作错误**

- **解决方案**：检查文件权限和路径，关闭占用文件的其他程序

**5. 浏览器打开失败**

- **解决方案**：检查默认浏览器设置，手动复制URL到浏览器访问

**6. 日志系统初始化失败**

- **解决方案**：程序继续运行，日志输出到控制台，检查磁盘空间和文件权限
...
ClassPaper v4 实现了自适应课程显示功能，根据实际课程数量动态调整显示布局，不再受固定位置限制。

#### 主要改进

- **动态计算显示位置**：根据实际课程数量计算中间位置，不再固定在第6位
- **灵活的课程数量**：支持任意数量的课程显示，不再限制为12个固定位置（day模式暂不支持）
- **智能休息显示**：休息时间标记插入合适位置，不会吞掉课程内容
- **自动清理多余位置**：自动清除不需要的显示位置，保持界面整洁
...
通过 GitHub API 获取所有 releases，自动识别当前运行的程序版本。

**核心特性**：自动获取所有 releases、智能版本识别、实时更新检查、详细状态显示

**工作原理**：获取所有 releases 并按发布时间排序，查找最接近但不超过编译日期的 release 作为当前版本，比较当前版本与最新版本判断是否有更新

**使用方法**：打开设置页面，切换到"关于"标签，点击"检查更新"按钮
...
- 实时预览：设置修改后即时预览
- 数据导入导出：支持课程表、配置的导入导出

### 🔧 如何体验新UI

前往 [GitHub Releases](https://github.com/E7G/Classpaper-v4/releases) 下载最新的beta版本。

### ⚠️ 注意事项

- dev分支包含开发中的功能，可能存在不稳定因素
- 遇到问题请通过 [GitHub Issues](https://github.com/E7G/Classpaper-v4/issues) 反馈
...
1. **配置文件测试**：删除 `config.toml` 文件，或将其设为只读
2. **文件权限测试**：以普通用户身份运行，尝试写入系统目录
3. **浏览器测试**：卸载Chrome/Edge，或修改配置文件中的无效浏览器路径
4. **查看错误日志**：所有错误都会记录在 `app.log` 中，包含详细的时间戳和错误堆栈信息

所有错误都会通过Windows原生弹窗显示，确保用户能够立即了解问题原因并获得具体的解决步骤。
